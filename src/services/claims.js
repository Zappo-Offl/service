...
const arbProvider = require('./nebula');
...
class ClaimsService {
  async createHold({ senderPhone, senderAddress, recipientPhone, amountArb, ephemeralWalletId, ephemeralWalletAddress, holdTxHash }) {
    try {
      // Validate input parameters
      if (!senderPhone || !senderAddress || !recipientPhone || !amountArb || !ephemeralWalletId || !ephemeralWalletAddress) {
        logger.error('Missing required parameters for createHold:', { 
          senderPhone, senderAddress, recipientPhone, amountArb, ephemeralWalletId, ephemeralWalletAddress 
        });
        throw new Error('Missing required parameters for creating escrow hold');
      }
      
      logger.info(`Creating escrow hold: ${senderPhone} -> ${recipientPhone}, amount: ${amountArb} ETH`);
  ...
  async validateAndClaim({ tokenPlain, claimerPhone, recipientWalletAddress }) {
    try {
      ...
      logger.info(`Estimating gas for claim: ${record.ephemeral_wallet_address} -> ${recipientWalletAddress}, amount: ${record.amount_arb}`);
      ...
      const gasEstimate = await arbProvider.estimateGas(record.ephemeral_wallet_address, recipientWalletAddress, record.amount_arb);
      ...
      const availableAmount = record.amount_arb;
      ...
      const tx = await arbProvider.sendTransaction(privyWallet, recipientWalletAddress, transferAmount);
  ...
  async refundExpired(refundHandler) {
    ...
    for (const rec of expired) {
      try {
        ...
        const gasEstimate = await arbProvider.estimateGas(rec.ephemeral_wallet_address, rec.sender_wallet_address, rec.amount_arb);
        const gasCost = parseFloat(gasEstimate.estimatedCost);
        const safetyBuffer = config.escrow.claimMinGasBuffer || 0.002;
        const totalRequired = gasCost + safetyBuffer;
        
        if (rec.amount_arb <= totalRequired) {
          logger.warn(`Refund failed: insufficient funds for gas. Required: ${totalRequired.toFixed(6)} ETH, Available: ${rec.amount_arb} ETH`);
        ...
        const refundAmount = rec.amount_arb - gasCost;
        
        const tx = await arbProvider.sendTransaction(privyWallet, rec.sender_wallet_address, refundAmount);
        await claims.updateClaimById(rec._id, { 
          status: 'refunded', 
  ...
  async createClaimLink(senderPhone, recipientPhone, amountArb, recipientName = 'Unknown') {
    try {
      logger.info(`Creating claim link: ${senderPhone} -> ${recipientPhone}, amount: ${amountArb} ETH`);
      ...
      try {
        ...
        // Use ETH provider to send transaction
        const transferResult = await arbProvider.sendTransaction(
          walletProvider,
