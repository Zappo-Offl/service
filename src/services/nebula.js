...
// Global provider instance for other services to use
...
class NebulaService {
  ...
  async initialize() {
    try {
      // Create provider for direct RPC calls (ETH C-Chain)
      const rpcUrl = process.env.ARB_RPC_URL || config.thirdweb.rpcUrl;
      this.provider = new ethers.providers.JsonRpcProvider(rpcUrl);
      arbProvider = this.provider;
      logger.info('✅ ARB provider initialized successfully');
    } catch (error) {
      logger.error('❌ Failed to initialize ARB provider:', error);
      throw error;
    }
  }
  ...
  async getBalance(address) {
    try {
      ...
      const balanceInArb = ethers.utils.formatEther(balance);
      
      logger.info(`Balance for ${address}: ${balanceInArb} ETH`);
      return {
        balance: balanceInArb,
  ...
  async estimateGas(from, to, amount) {
    try {
      ...
      const estimatedGasCostArb = ethers.utils.formatEther(estimatedGasCost);
      
      return {
        gasLimit: gasEstimate.toString(),
        gasPrice: gasPrice.toString(),
        estimatedCost: estimatedGasCostArb,
  ...
  async sendTransaction(wallet, to, amount, gasLimit = null) {
    try {
      ...
      if (wallet.isPrivyWallet) {
        ...
        tx.chainId = 421614; // ETH Sepolia Testnet
