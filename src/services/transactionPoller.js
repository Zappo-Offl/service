...
class TransactionPoller {
  ...
  async pollTransactions() {
    try {
      ...
      //     await this.whatsapp.sendMessage(whatsappPhone, { text: `⏳ Reminder: ${rec.amount_arb} ETH pending claim for +${rec.recipient_phone} (${daysLeft} day(s) left).` });
      //   } catch (e) {
      ...
      await claimsService.refundExpired(async (rec, tx, gasCost, refundAmount) => {
        try {
          const whatsappPhone = `${rec.sender_phone}@s.whatsapp.net`;
          const originalAmount = rec.amount_arb;
          
          const message = `↩️ *Refund Processed*

💰 *Original Amount:* ${originalAmount} ETH
⛽ *Gas Fee:* ${gasCost.toFixed(6)} ETH
💸 *Amount Refunded:* ${refundAmount.toFixed(6)} ETH
🔗 *Transaction Hash:* \`${tx.hash}\`
📊 *View on Arbitrum Sepolia:* https://sepolia.arbiscan.io/tx/${tx.hash}
...
*Note: Gas fees were deducted to process your refund.*`;
  ...
  async confirmPendingTransactions() {
    try {
      ...
      for (const tx of pending) {
        try {
          const status = await arbProvider.getTransactionStatus(tx.tx_hash);
  ...
  async checkUserTransactions(user) {
    try {
      ...
      const currentBlock = await arbProvider.provider.getBlockNumber();
  ...
  async scanBlocksForTransactions(walletAddress, fromBlock, toBlock) {
    ...
    try {
      ...
      for (let blockNumber = fromBlock; blockNumber <= actualToBlock; blockNumber++) {
        try {
          const block = await arbProvider.provider.getBlock(blockNumber, true);
          ...
          for (const tx of block.transactions) {
            ...
            if (tx.to && tx.to.toLowerCase() === walletAddress.toLowerCase() && 
              ...
              transactions.push({
                ...
                from: tx.from,
                to: tx.to,
                value: arbProvider.formatARB(tx.value),
  ...
  async processIncomingTransaction(user, transaction) {
    try {
      ...
      console.log('Logging transaction:', txData);
      await transactions.createTransaction(txData);
      console.log('Transaction logged successfully');
      console.log(transactions.findPendingTransactions(txData.tx_hash));
...
🔍 [View on Explorer](https://sepolia.arbiscan.io/tx/${transaction.hash})
