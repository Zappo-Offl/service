...
class CommandParser {
  constructor() {
    this.intents = {
      ...
      // Send commands
      SEND_ARB: /^(send|transfer|pay)\s+(\d+(?:\.\d+)?)\s*(ETH|arbitrum)?\s*(to|@)?\s*(.+)/i,
      SEND_ARB_START: /^(send|transfer|pay)\s+(ETH|arbitrum)$/i,
  ...
  parseSendParameters(args) {
    try {
      // Pattern: "amount ETH to recipient"
      const sendPattern = /^(\d+(?:\.\d+)?)\s*(ETH|arbitrum)?\s*(to|@)?\s*(.+)/i;
      ...
      if (amountMatch) {
        ...
        const recipient = args.replace(amountMatch[0], '').replace(/\s*(ETH|arbitrum|to|@)\s*/gi, '').trim();
      ...
      return { valid: false, error: 'Invalid send format. Use: amount ETH to recipient' };
  ...
  validateAmount(amount) {
    return !isNaN(amount) && amount > 0 && amount <= 1000000; // Max 1M ETH
  }
...
*Balance & History:*
• \`/balance\` - Check testnet ETH balance
• \`/history\` - View recent transactions

*Send ETH:*
• \`/send amount ETH to address\`
• "send 1 ETH to 0x..." or "transfer 0.5 to John"
• "send ETH" - Step-by-step send flow
...
*Get Testnet ETH:*
🔗 [Free Faucet](https://arbitrum.faucet.dev/ArbSepolia)
