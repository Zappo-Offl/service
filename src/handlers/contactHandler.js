...
const arbProvider = require('../services/nebula');
...
class ContactHandler {
  ...
  async handleAddContact(from, phone, parameters) {
    try {
      ...
      if (!arbProvider.validateAddress(address)) {
...
You can now send ETH to “${name}” without typing the full address.`);
...
Tip: Use /addcontact name 0xaddress to save one for easier sending.`);
      ...
      contactsMessage += `💡 *Usage:* Send "send 1 ETH to ${userContacts[0].name}" to use contacts!`;
...
*Using Contacts:*
• "send 1 ETH to John" (instead of full address)
• "transfer 0.5 to John"

💡 *Tip:* Contacts make sending ETH much easier!`;
  }
